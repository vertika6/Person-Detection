...
def draw_custom_boxes(detections, image, colors, sort_tracked_object):
    import cv2
    #index=0
    for index, (label, confidence, bbox) in enumerate(detections):
        print("sort",sort_tracked_object)
        print("detect",detections)
    #try:
        id = sort_tracked_object[index][-1]
    #except:
        #print('*'*21)
        print(detections)
        ...
        left, top, right, bottom = bbox2points(bbox)
        cv2.rectangle(image, (left, top), (right, bottom), colors[label], 1)
        cv2.putText(image, "{} [{:.2f}] | ID: {}".format(label, float(confidence), int(id)),
                    (left, top - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                    colors[label], 2)
    return image
